...
public class Chat {
    ...
    public static void requestInput(Plugin plugin, Player player, String prompt,
        ...
        ConversationFactory factory = new ConversationFactory(plugin).withModality(true)
                .withLocalEcho(false).withPrefix(context -> "§6§l[Input] §r")
     ...
     * @param onCancel Optional callback for when input is cancelled
     */
    public static void requestInput(Plugin plugin, Player player, Component prompt,
            Predicate<String> validator, Consumer<String> onInput, Runnable onCancel) {

        requestInput(plugin, player, LegacyComponentSerializer.legacySection().serialize(prompt),
                validator, onInput, onCancel);
    }
     ...
     */
    public static void requestNumericInput(Plugin plugin, Player player, String prompt,
            Consumer<Double> onInput) {
        requestInput(plugin, player, prompt, input -> {
            try {
