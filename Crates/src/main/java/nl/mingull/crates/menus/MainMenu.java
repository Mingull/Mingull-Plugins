...
public class MainMenu extends Menu {
	...
	public Map<Integer, Icon> getIcons() {
		...
		icons.put(3, Icons.createIcon(Material.CHEST, Messenger.format("<gold>List Crates"),
				Messenger.format("<gray>Click to view a list of all crates")).setAction(p -> {
					try{
		...
		icons.put(5, Icons.createIcon(Material.EMERALD, Messenger.format("<gold>Create Crate"),
				Messenger.format("<gray>Click to create a new crate")).setAction(p -> {
					p.closeInventory();
					ChatManager.requestInput(p,
							Messenger.format("<blue>Enter a name of the new crate (or 'cancel' to cancel):"), input -> {
								if (plugin.getManager(CrateManager.class).crateExists(input)){
									p.sendMessage(Messenger.format("<red>A crate with that name already exists."));
									return false;
								}
								return true;
							}, input -> {
								plugin.getManager(CrateManager.class).createCrate(input);
								p.sendMessage(Messenger
										.format("<green>Successfully <blue>created crate <gold>" + input + "</gold>."));

								p.sendMessage(Messenger.format("<blue>Click <click:run_command:'/crates edit " + input
										+ "'><hover:show_text:'<gray>Click to manage crate'><gold>here</gold></hover></click> to manage the crate."));
							}, () -> {
								p.sendMessage(Messenger.format("<red>Crate creation cancelled."));
							});
				}));
		return icons;
	}
